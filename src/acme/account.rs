...
pub(crate) mod test {
    ...
    pub(crate) fn parse_req(
        ...
        let body: HashMap<String, String> = serde_json::from_str(parts.1).expect("body not json");
        ...
        let payload = if payload.is_empty() {
            ...
            let payload: serde_json::Map<String, serde_json::Value> =
                serde_json::from_slice(&B64_URL_SAFE_NO_PAD.decode(payload).expect("b64"))
    ...
    fn send_csr() {
        async fn server(listener: TcpListener) -> std::io::Result<bool> {
            ...
            let i = payload
                ...
                .as_str()
                .expect("csr not str");
            let i = B64_URL_SAFE_NO_PAD.decode(i).expect("b64");
        ...
        block_on(async {
            ...
            let account = new_account(directory);
            let o = account
                .send_csr(format!("http://{host}:{port}/csr"), b"csr".to_vec())
                .await?;

            let c = match o {
                Order::Valid { certificate } => certificate,
                _ => panic!("wrong variant"),
            };
